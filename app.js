// 📂 ganadero-elite/app.js - GANADERO ÉLITE v5.0 UNIFICADO
console.log('🧠 GANADERO ÉLITE v5.0 - IA PREDICTIVA TOTAL + CICLO REPRODUCTIVO');
...
function getIconoAnimal(a) {
    if (a.foto && a.foto.length > 100) return '<img src="' + a.foto + '" alt="' + a.nombre + '" style="width:100%;height:100%;object-fit:cover;">';
...
var DB = { 
    ...
    precioKG: 9800, litroLeche: 1500 
};
var fotosDB = {};

function cargarDatos() { 
    try { var s = localStorage.getItem('ganadero_elite_v9'); if (s) { DB = JSON.parse(s); } } catch(e) {} 
    try { var f = localStorage.getItem('ganadero_fotos'); if (f) { fotosDB = JSON.parse(f); for (var i = 0; i < DB.animales.length; i++) { if (fotosDB[DB.animales[i].id]) DB.animales[i].foto = fotosDB[DB.animales[i].id]; } } } catch(e) {} 
}
function save() { 
    try { 
        var copia = JSON.parse(JSON.stringify(DB)); 
        for (var i = 0; i < copia.animales.length; i++) { 
            if (copia.animales[i].foto && copia.animales[i].foto.length > 100) { 
                fotosDB[copia.animales[i].id] = copia.animales[i].foto; 
                copia.animales[i].foto = '[FOTO]'; 
            } 
        } 
        localStorage.setItem('ganadero_elite_v9', JSON.stringify(copia)); 
        localStorage.setItem('ganadero_fotos', JSON.stringify(fotosDB)); 
    } catch(e) {} 
}
...
function getDietaCompleta(pv, tipo, estadoRepro) {
    ...
    var melazaKg = consumoTotal * (m.melaza / 100);
    return { pasto: consumoTotal * 0.90, salvado: consumoTotal * 0.10, melaza: melazaKg * 1000, urea: (pv < 150 && tipo === 'engorde') ? 0 : (pv * m.urea), bicarb: pv * m.bicarb, sal: pv * m.sal, levadura: pv * 0.05, consumoTotal: consumoTotal };
}
...
function getRendimiento(h) { 
    ...
    if (act < ant) return { nivel:'gris', texto:'Pérdida', icono:'fa-circle-exclamation', cm:cm, color:'gris', tendencia:tend }; 
...
function predecirPeso(historial, diasFuturo) { 
    ...
    return m * (Math.floor((fu - fb) / 86400000) + diasFuturo) + b; 
}

function getConfianzaPrediccion(historial) { 
    if (historial.length < 3) return 'Baja'; 
    var cm = []; for (var i = 1; i < historial.length; i++) cm.push(historial[i].peso - historial[i-1].peso); 
    var med = cm.reduce(function(a,b){return a+b;},0)/cm.length; 
    if (med === 0) return 'Baja'; 
    var vr = cm.reduce(function(a,b){return a+Math.pow(b-med,2);},0)/cm.length; 
    var cv = Math.sqrt(vr)/Math.abs(med); 
    if (cv < 0.3) return 'Alta'; if (cv < 0.6) return 'Media'; return 'Baja'; 
}

function getTendenciaTexto(historial) { 
    if (historial.length < 2) return '📊 Estable'; 
    var c = 0; for (var i = 1; i < historial.length; i++) { if (historial[i].peso > historial[i-1].peso) c++; else if (historial[i].peso < historial[i-1].peso) c--; } 
    if (c > 0) return '📈 Mejorando'; if (c < 0) return '📉 Empeorando'; return '📊 Estable'; 
}

function getSemaforo(animal) { 
    if (animal.tipo !== 'leche' || animal.estadoRepro !== 'parida' || !animal.fechaParto) return null; 
    var diasPostParto = getDiasDesde(animal.fechaParto); 
...
function getAlertasLote() {
    ...
    DB.animales.forEach(function(a) { 
        var du = getDiasDesde(a.historial[a.historial.length-1].fecha); 
        if (du > 30) alertas.push({ t:'w', m:'<b>' + a.nombre + '</b>: Pesaje vencido (' + du + 'd)', icon:'fa-calendar-xmark' }); 
        var ti = false; for (var i = DB.aplicaciones.length-1; i >= 0; i--) { if (DB.aplicaciones[i].animalId === a.id && (DB.aplicaciones[i].productoId === 'ivermectina1' || DB.aplicaciones[i].productoId === 'ivermectina315') && getDiasDesde(DB.aplicaciones[i].fecha) < 90) { ti = true; break; } } 
        if (!ti && a.historial[a.historial.length-1].peso >= 150) alertas.push({ t:'purple', m:'<b>' + a.nombre + '</b>: Desparasitación vencida', icon:'fa-shield-virus' }); 
        var cat = getCatalogoSanidadCompleto(); for (var j = DB.aplicaciones.length-1; j >= 0; j--) { if (DB.aplicaciones[j].animalId === a.id) { var pr = cat.find(function(p) { return p.id === DB.aplicaciones[j].productoId; }); if (pr && pr.retiro > 0 && getDiasDesde(DB.aplicaciones[j].fecha) < pr.retiro) { alertas.push({ t:'r', m:'<b>' + a.nombre + '</b>: Retiro ' + (pr.retiro - getDiasDesde(DB.aplicaciones[j].fecha)) + 'd', icon:'fa-clock' }); break; } } } 
    });
    return alertas;
}
// ==================== NAVEGACIÓN ====================
...
function addAnimal() { 
    ...
    var origen = document.getElementById('newOrigen').value; 
    var loteId = document.getElementById('newLote').value; 
    var animal = { id: Date.now(), nombre: n, tipo: tipo, origen: origen, historial: [{ fecha: new Date().toLocaleDateString(), peso: p }], lote: loteId || null, foto: null }; 
    if (origen === 'nacimiento') { animal.madre = document.getElementById('newMadre').value.trim() || null; animal.fechaNacimiento = new Date().toLocaleDateString(); } 
    if (origen === 'comprado') { animal.precioCompra = parseFloat(document.getElementById('newPrecio').value) || 0; animal.fechaCompra = new Date().toLocaleDateString(); } 
    if (tipo === 'leche') { animal.estadoRepro = (p < 350) ? 'novilla' : 'parida'; animal.produccionLeche = []; } 
    DB.animales.push(animal); save(); closeAddModal(); renderDashboard(); showToast('<i class="fa-solid fa-check-circle"></i> ' + n + ' registrado'); 
}

// ==================== RENDER DASHBOARD ====================
function renderDashboard() {
    var price = DB.precioKG, totalKg = 0, mez = { pasto:0, salvado:0, sal:0, melaza:0, urea:0, levadura:0, bicarb:0 };
    var costoTotal = 0, est = { verde:0, azul:0, naranja:0, rojo:0, gris:0 }, csTotal = 0, pesoProy30 = 0;
    ...
    for (var i = 0; i < DB.lotes.length; i++) {
        ...
        var kgL = 0, cdL = 0, gmdL = 0, cg = 0, litrosL = 0;
        for (var j = 0; j < animalesLote.length; j++) { 
            var a = animalesLote[j], cp = a.historial[a.historial.length-1].peso; kgL += cp; 
            cdL += getCostoDiario(cp, a.tipo, a.estadoRepro); 
            var g = getGMD(a.historial); if (a.historial.length >= 2) { gmdL += g; cg++; } 
            if (a.tipo === 'leche' && a.produccionLeche && a.produccionLeche.length > 0) litrosL += a.produccionLeche[a.produccionLeche.length-1].litros; 
        }
        var pGMD = cg > 0 ? gmdL / cg : 0, valorL = kgL * price, ingL = pGMD * 30 * price * animalesLote.length, ganL = ingL - (cdL * 30);
        metricasLotes.push({ lote:lote, animales:animalesLote.length, kg:kgL, valor:valorL, gmd:pGMD, costoDiario:cdL, ganancia:ganL, litros:litrosL });
    }
    metricasLotes.sort(function(a,b) { return b.ganancia - a.ganancia; });
    var animalesSinLote = DB.animales.filter(function(a) { return !a.lote; }), kgSL = 0; for (var i = 0; i < animalesSinLote.length; i++) kgSL += animalesSinLote[i].historial[animalesSinLote[i].historial.length-1].peso;
    ...
    var ta = DB.animales.length, prom = ta > 0 ? totalKg/ta : 0, gmdL = ta > 0 ? DB.animales.reduce(function(s,a) { return s + getGMD(a.historial); },0)/ta : 0;
    ...
    for (var i = 0; i < metricasLotes.length; i++) {
        ...
        var pctProgreso = Math.min(100, Math.max(0, (m.ganancia / (Math.abs(m.ganancia) + Math.abs(m.costoDiario*30))) * 100));
        lotesHTML += '<div class="lote-card" onclick="verLote(\'' + m.lote.id + '\')"><div class="lote-nombre">' + tipoIcono + ' ' + m.lote.nombre + ' <span style="font-size:.6rem;color:var(--muted);">(' + m.animales + ' anim.)</span></div><div class="lote-stats"><div class="lote-stat"><div class="val">' + fm(m.kg) + ' kg</div><div class="lbl">Peso total</div></div><div class="lote-stat"><div class="val">' + m.gmd.toFixed(2) + '</div><div class="lbl">GMD</div></div><div class="lote-stat"><div class="val" style="color:' + (m.ganancia >= 0 ? '#22c55e' : '#ef4444') + '">' + (m.ganancia >= 0 ? '+' : '') + '$' + fm(m.ganancia) + '</div><div class="lbl">Ganancia/mes</div></div>' + (m.litros > 0 ? '<div class="lote-stat"><div class="val">' + m.litros + ' L</div><div class="lbl">Leche/día</div></div>' : '<div class="lote-stat"><div class="val"></div><div class="lbl"></div></div>') + '</div><div class="progress-lote"><div class="progress-lote-fill" style="width:' + pctProgreso + '%;"></div></div></div>';
    }
    if (animalesSinLote.length > 0) lotesHTML += '<div class="lote-card" onclick="verAnimalesSinLote()"><div class="lote-nombre">📋 SIN LOTE <span style="font-size:.6rem;color:var(--muted);">(' + animalesSinLote.length + ')</span></div><div class="lote-stats"><div class="lote-stat"><div class="val">' + fm(kgSL) + ' kg</div></div></div></div>';
    ...
    // Alertas
    var alHTML = '';
    if (alertasL.length > 0) { alHTML = '<div class="card card-sm"><div style="font-weight:700;font-size:.68rem;margin-bottom:6px;color:var(--muted);"><i class="fa-solid fa-bell"></i> ALERTAS (' + alertasL.length + ')</div>'; for (var x = 0; x < Math.min(alertasL.length, 4); x++) { var cls = alertasL[x].t === 'r' ? 'alert-danger' : alertasL[x].t === 'purple' ? 'alert-purple' : alertasL[x].t === 'info' ? 'alert-info' : 'alert-warning'; alHTML += '<div class="alert-item ' + cls + '"><i class="fa-solid ' + (alertasL[x].icon || 'fa-circle') + '"></i> ' + alertasL[x].m + '</div>'; } if (alertasL.length > 4) alHTML += '<div style="font-size:.6rem;color:var(--muted);text-align:center;">+' + (alertasL.length - 4) + ' más</div>'; alHTML += '</div>'; }

    // IA del lote
    var iaHTML = '';
    if (pesoProy30 > 0) { iaHTML = '<div class="ia-card"><div class="ia-title"><i class="fa-solid fa-brain"></i> PREDICCIÓN IA DEL LOTE</div><div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;margin-bottom:6px;"><div class="proyeccion-item"><div class="dias">PESO HOY</div><div class="peso">' + fm(totalKg) + ' kg</div><div class="ganancia">$ ' + fm(totalKg * price) + '</div></div><div class="proyeccion-item"><div class="dias">30 DÍAS</div><div class="peso">' + fm(pesoProy30) + ' kg</div><div class="ganancia" style="color:' + (gan30 >= 0 ? '#22c55e' : '#ef4444') + '">' + (gan30 >= 0 ? '+' : '') + '$ ' + fm(gan30) + '</div></div><div class="proyeccion-item"><div class="dias">TENDENCIA</div><div class="peso">' + getTendenciaTexto(DB.animales.length > 0 ? DB.animales[0].historial : []) + '</div><div class="ganancia">GMD: ' + gmdL.toFixed(2) + '</div></div></div>'; if (mejorA) iaHTML += '<div class="ranking-item" onclick="showProfile(' + mejorA.id + ')" style="cursor:pointer;"><span class="ranking-emoji">🏆</span> Mejor: <b>' + mejorA.nombre + '</b> (+' + mejorA.gmd.toFixed(2) + ' kg/d)</div>'; if (peorA && ta > 1) iaHTML += '<div class="ranking-item" onclick="showProfile(' + peorA.id + ')" style="cursor:pointer;"><span class="ranking-emoji">⚠️</span> Atención: <b>' + peorA.nombre + '</b> (+' + peorA.gmd.toFixed(2) + ' kg/d)</div>'; iaHTML += '</div>'; }

    var mezHTML = ''; for (var z = 0; z < ALIMENTOS.length; z++) mezHTML += '<div class="row"><span class="row-label"><i class="fa-solid ' + IC_ALIMENTOS[ALIMENTOS[z]] + '"></i> ' + NM_ALIMENTOS[ALIMENTOS[z]] + '</span><span class="row-val">' + (ALIMENTOS[z] === 'pasto' || ALIMENTOS[z] === 'salvado' ? mez[ALIMENTOS[z]].toFixed(1) + ' kg' : Math.round(mez[ALIMENTOS[z]]) + ' g') + '</span></div>';

    var html = '<div class="card card-sm card-accent"><div style="font-weight:600;"><i class="fa-solid fa-coins"></i> PRECIO KG EN PIE</div><div style="display:flex;align-items:center;gap:6px;"><span style="font-size:1.1rem;font-weight:800;color:var(--accent);">$</span><input id="inpPKG" type="number" value="' + price + '" style="font-size:1.1rem;font-weight:700;text-align:center;"><span style="font-size:.7rem;color:var(--muted);">COP</span></div><button class="btn btn-green mt8" onclick="savePKG()"><i class="fa-solid fa-check"></i> ACTUALIZAR</button></div>' +
        ...
        '<div class="section-title"><i class="fa-solid fa-layer-group"></i> LOTES</div>' + lotesHTML + iaHTML + alHTML +
...
function savePKG() { var el = document.getElementById('inpPKG'); if (el) { DB.precioKG = parseFloat(el.value) || 0; save(); renderDashboard(); showToast('<i class="fa-solid fa-check-circle"></i> Precio actualizado'); } }
...
function renderAnimalesGrid(animales, titulo) {
    ...
    for (var i = 0; i < animales.length; i++) { 
        var a = animales[i], cp = a.historial[a.historial.length-1].peso, etapa = getEtapaCompleta(cp, a.tipo, a.estadoRepro), 
            r = getRendimiento(a.historial), lm = { verde:'ml-g', azul:'ml-b', naranja:'ml-o', rojo:'ml-r', gris:'ml-x' }, 
            sg = r.cm >= 0 ? '+' : '', sem = getSemaforo(a), 
            semHTML = sem ? '<span class="semaforo semaforo-' + sem.color + '"></span>' : ''; 
        cards += '<div class="animal-card ' + etapa.cardClass + '" onclick="showProfile(' + a.id + ')"><div class="mini-led ' + lm[r.nivel] + '"></div><div class="animal-avatar">' + getIconoAnimal(a) + '</div><div class="name">' + a.nombre + ' ' + semHTML + '</div><span class="etapa-tag ' + etapa.clase + '">' + etapa.rango + '</span><div class="weight">' + fm(cp) + ' kg</div><div class="cm" style="color:' + (r.cm >= 0 ? '#22c55e' : '#ef4444') + '">' + sg + r.cm.toFixed(1) + '%</div></div>'; 
    }
    var html = '<div class="card"><div style="display:flex;justify-content:space-between;align-items:center;"><div style="font-weight:700;font-size:.8rem;color:var(--accent);">' + titulo + ' (' + animales.length + ')</div><button class="btn btn-gray btn-sm" onclick="renderDashboard()"><i class="fa-solid fa-arrow-left"></i> Volver</button></div></div><div class="grid">' + cards + '</div>';
    document.getElementById('v-lote').innerHTML = html;
}
// ==================== PERFIL DEL ANIMAL ====================
function showProfile(id) {
    ...
    var html = '<div class="card"><div class="profile-cover"><div class="profile-avatar" onclick="abrirFoto(' + id + ')">' + getIconoAnimal(a) + '<div class="foto-overlay">📸</div></div><div class="profile-name">' + a.nombre + '</div><div class="profile-sub">' + etapa.rango + ' · ' + (a.tipo === 'engorde' ? '🥩 Engorde' : '🥛 Leche');
    ...
    html += '<div class="card card-sm"><div style="font-weight:700;font-size:.65rem;margin-bottom:4px;color:var(--muted);"><i class="fa-solid fa-calculator"></i> RENTABILIDAD</div><div class="row"><span class="row-label"><i class="fa-solid fa-receipt"></i> Costo alim./día</span><span class="row-val">$ ' + fm(cd) + '</span></div><div class="row"><span class="row-label"><i class="fa-solid fa-sack-dollar"></i> Ganancia/mes</span><span class="row-val" style="color:' + (gan >= 0 ? '#22c55e' : '#ef4444') + '">$ ' + fm(gan) + '</span></div></div>' +
        ...
        '<div style="display:flex;gap:8px;margin-top:14px;flex-wrap:wrap;">' + botonesHTML + '<button class="btn btn-purple btn-sm" onclick="openAplicarSanidad(' + id + ')" style="flex:1;"><i class="fa-solid fa-syringe"></i> Sanidad</button><button class="btn btn-gold btn-sm" onclick="updateWeight(' + id + ')" style="flex:2;"><i class="fa-solid fa-gauge-high"></i> REGISTRAR PESAJE</button></div>';
    document.getElementById('v-perfil').innerHTML = html; window.scrollTo(0, 0); save();
}

// ==================== FOTO / CAMBIAR LOTE / LECHE / REPRODUCTIVAS ====================
function abrirFoto(id) { var input = document.getElementById('fotoInput'); input.setAttribute('data-animal', id); input.click(); }
function guardarFoto() { var input = document.getElementById('fotoInput'); var id = parseInt(input.getAttribute('data-animal')); var file = input.files[0]; if (!file) return; var reader = new FileReader(); reader.onload = function(e) { var a = DB.animales.find(function(x) { return x.id === id; }); if (!a) return; a.foto = e.target.result; fotosDB[id] = e.target.result; save(); showProfile(id); showToast('<i class="fa-solid fa-camera"></i> Foto guardada'); }; reader.readAsDataURL(file); }

function cambiarLote(id) {
    var a = DB.animales.find(function(x) { return x.id === id; }); if (!a) return;
    var loteActual = DB.lotes.find(function(l) { return l.id === a.lote; });
    var html = '<div class="modal-title"><i class="fa-solid fa-rotate"></i> CAMBIAR DE LOTE - ' + a.nombre + '</div>';
    html += '<div style="font-size:.7rem;color:var(--muted);margin-bottom:8px;">Lote actual: ' + (loteActual ? loteActual.nombre : 'Sin lote') + '</div>';
    html += '<div class="flex-col gap10"><div style="font-weight:600;font-size:.7rem;color:var(--accent);">LOTES COMPATIBLES (' + (a.tipo === 'engorde' ? '🥩 Engorde' : '🥛 Leche') + '):</div>';
    var lotesCompatibles = DB.lotes.filter(function(l) { return l.tipo === a.tipo; });
    for (var i = 0; i < lotesCompatibles.length; i++) { html += '<button class="btn btn-sm ' + (a.lote === lotesCompatibles[i].id ? 'btn-gold' : 'btn-gray') + '" onclick="confirmarCambioLote(' + id + ',\'' + lotesCompatibles[i].id + '\')" style="width:100%;text-align:left;justify-content:flex-start;">' + (a.lote === lotesCompatibles[i].id ? '✅ ' : '') + lotesCompatibles[i].nombre + '</button>'; }
    html += '<button class="btn btn-sm ' + (!a.lote ? 'btn-gold' : 'btn-gray') + '" onclick="confirmarCambioLote(' + id + ',null)" style="width:100%;text-align:left;justify-content:flex-start;">' + (!a.lote ? '✅ ' : '') + '📋 Sin lote</button>';
    html += '<div style="font-weight:600;font-size:.7rem;color:var(--accent);margin-top:8px;">CAMBIAR A OTRO TIPO:</div>';
    var otroTipo = a.tipo === 'engorde' ? 'leche' : 'engorde';
    var lotesOtroTipo = DB.lotes.filter(function(l) { return l.tipo === otroTipo; });
    for (var j = 0; j < lotesOtroTipo.length; j++) { html += '<button class="btn btn-sm btn-gray" onclick="confirmarCambioLoteTipo(' + id + ',\'' + lotesOtroTipo[j].id + '\',\'' + otroTipo + '\')" style="width:100%;text-align:left;justify-content:flex-start;">' + (otroTipo === 'engorde' ? '🥩 ' : '🥛 ') + lotesOtroTipo[j].nombre + '</button>'; }
    html += '<button class="btn btn-sm btn-gray" onclick="confirmarCambioLoteTipo(' + id + ',null,\'' + otroTipo + '\')" style="width:100%;text-align:left;justify-content:flex-start;">📋 Sin lote (' + (otroTipo === 'engorde' ? '🥩' : '🥛') + ')</button>';
    html += '<button class="btn btn-gray btn-sm mt8" onclick="document.querySelector(\'.modal-overlay\').remove()">CANCELAR</button></div>';
    showModal(html);
}
function confirmarCambioLote(id, loteId) { var a = DB.animales.find(function(x) { return x.id === id; }); if (!a) return; a.lote = loteId; save(); document.querySelector('.modal-overlay').remove(); showProfile(id); showToast('<i class="fa-solid fa-check-circle"></i> Lote actualizado'); }
function confirmarCambioLoteTipo(id, loteId, nuevoTipo) {
    if (!confirm('⚠️ ¿Cambiar este animal a ' + (nuevoTipo === 'engorde' ? '🥩 Engorde' : '🥛 Leche') + '?\n\nLa dieta se ajustará automáticamente.')) return;
    var a = DB.animales.find(function(x) { return x.id === id; }); if (!a) return;
    a.tipo = nuevoTipo; a.lote = loteId;
    if (nuevoTipo === 'engorde') { a.estadoRepro = 'venta'; a.produccionLeche = []; }
    else { a.estadoRepro = 'novilla'; }
    save(); document.querySelector('.modal-overlay').remove(); showProfile(id); showToast('<i class="fa-solid fa-check-circle"></i> Tipo y lote actualizados');
}

function registrarLeche(id) { var a = DB.animales.find(function(x) { return x.id === id; }); if (!a) return; var litrosHoy = a.produccionLeche && a.produccionLeche.length > 0 ? a.produccionLeche[a.produccionLeche.length-1].litros : 0; var html = '<div class="modal-title"><i class="fa-solid fa-glass-water-droplet"></i> REGISTRAR LECHE - ' + a.nombre + '</div><div class="flex-col gap10"><input id="lecheLitros" type="number" placeholder="Litros producidos hoy" step="0.5" value="' + litrosHoy + '"><input id="lechePrecio" type="number" placeholder="Precio por litro ($)" value="' + (DB.litroLeche || 1500) + '"><button class="btn btn-gold mt8" onclick="guardarLeche(' + id + ')"><i class="fa-solid fa-check"></i> GUARDAR</button><button class="btn btn-gray" onclick="document.querySelector(\'.modal-overlay\').remove()">CANCELAR</button></div>'; showModal(html); }
function guardarLeche(id) { var litros = parseFloat(document.getElementById('lecheLitros').value); var precio = parseFloat(document.getElementById('lechePrecio').value); if (isNaN(litros) || litros < 0) { alert('⚠️ Litros válidos'); return; } if (precio && !isNaN(precio)) DB.litroLeche = precio; var a = DB.animales.find(function(x) { return x.id === id; }); if (!a) return; if (!a.produccionLeche) a.produccionLeche = []; a.produccionLeche.push({ fecha: new Date().toLocaleDateString(), litros: litros }); save(); document.querySelector('.modal-overlay').remove(); showProfile(id); showToast('<i class="fa-solid fa-check-circle"></i> ' + litros + ' L registrados'); }

function quedoPrenada(id) { if (!confirm('🤰 ¿Confirmar que esta vaca quedó preñada?\n\nSe calculará la fecha de parto (285 días) y la fecha de secado (60 días antes).')) return; var a = DB.animales.find(function(x) { return x.id === id; }); if (!a) return; var hoy = new Date(); a.fechaPrenez = hoy.toLocaleDateString(); var parto = new Date(hoy.getTime() + 285 * 86400000); a.fechaPartoEstimada = parto.toLocaleDateString(); var secado = new Date(parto.getTime() - 60 * 86400000); a.fechaSecado = secado.toLocaleDateString(); save(); showProfile(id); showToast('<i class="fa-solid fa-check-circle"></i> Preñez registrada · Parto: ' + a.fechaPartoEstimada); }
function iniciarSecado(id) { if (!confirm('🔴 ¿Iniciar secado de esta vaca?\n\nCambiará a dieta de Seca (1% Melaza, 0.20g Sal).')) return; var a = DB.animales.find(function(x) { return x.id === id; }); if (!a) return; a.estadoRepro = 'seca'; a.fechaSecadoInicio = new Date().toLocaleDateString(); save(); showProfile(id); showToast('<i class="fa-solid fa-check-circle"></i> Secado iniciado · Dieta ajustada'); }
function yaPario(id) { if (!confirm('✅ ¿Confirmar que esta vaca ya parió?\n\nSe reiniciará el ciclo reproductivo.')) return; var a = DB.animales.find(function(x) { return x.id === id; }); if (!a) return; a.estadoRepro = 'parida'; a.fechaParto = new Date().toLocaleDateString(); a.fechaPrenez = null; a.fechaPartoEstimada = null; a.fechaSecado = null; a.fechaSecadoInicio = null; if (confirm('🐮 ¿Desea registrar la cría recién nacida?')) { var pesoCria = parseFloat(prompt('⚖️ Peso de la cría al nacer (kg):')); if (!isNaN(pesoCria) && pesoCria > 10) { var nombreCria = prompt('📝 Nombre de la cría:') || ('Cría de ' + a.nombre); DB.animales.push({ id: Date.now() + 1, nombre: nombreCria, tipo: 'leche', origen: 'nacimiento', madre: a.nombre, fechaNacimiento: new Date().toLocaleDateString(), historial: [{ fecha: new Date().toLocaleDateString(), peso: pesoCria }], estadoRepro: 'novilla', lote: a.lote, produccionLeche: [] }); } } save(); showProfile(id); showToast('<i class="fa-solid fa-check-circle"></i> Parto registrado · Ciclo reiniciado'); }
function cambiarAEngorde(id) { if (!confirm('🔄 ¿Cambiar esta vaca a ENGORDE para venta?\n\nSe aplicará dieta de ceba (5% Melaza, 0.11g Urea).')) return; var a = DB.animales.find(function(x) { return x.id === id; }); if (!a) return; a.tipo = 'engorde'; a.estadoRepro = 'venta'; save(); showProfile(id); showToast('<i class="fa-solid fa-check-circle"></i> Cambiada a Engorde · Dieta ajustada'); }

// ==================== ACCIONES DEL PERFIL ====================
function updateWeight(id) { var p = prompt('⚖️ Nuevo pesaje (kg):'); if (!p) return; p = parseFloat(p); if (isNaN(p) || p < 20 || p > 2000) { alert('⚠️ Peso 20-2000 kg'); return; } var a = DB.animales.find(function(x) { return x.id === id; }); if (!a) return; a.historial.push({ fecha: new Date().toLocaleDateString(), peso: p }); save(); showProfile(id); showToast('<i class="fa-solid fa-check-circle"></i> Pesaje guardado'); }
function deleteAnimal(id) { if (confirm('⚠️ ¿Eliminar este animal?')) { DB.animales = DB.animales.filter(function(x) { return x.id !== id; }); save(); goPage('lote'); showToast('<i class="fa-solid fa-trash"></i> Animal eliminado'); } }
...
function openAgregarSuplementoSanidad() { var html = '<div class="modal-title"><i class="fa-solid fa-syringe"></i> NUEVO INYECTABLE</div><div class="flex-col gap10"><input id="supSanNombre" type="text" placeholder="Nombre"><input id="supSanDosis" type="number" placeholder="Dosis (ml/kg)" step="1" value="50"><input id="supSanDiasEfecto" type="number" placeholder="Días de efecto" step="1" value="30"><input id="supSanRetiro" type="number" placeholder="Días de retiro" step="1" value="0"><button class="btn btn-purple mt8" onclick="agregarSuplementoSanidad()"><i class="fa-solid fa-check"></i> AGREGAR</button><button class="btn btn-gray" onclick="document.querySelector(\'.modal-overlay\').remove()">CANCELAR</button></div>'; showModal(html); }
function agregarSuplementoSanidad() { var n = document.getElementById('supSanNombre').value.trim(), dosis = parseFloat(document.getElementById('supSanDosis').value) || 50, de = parseInt(document.getElementById('supSanDiasEfecto').value) || 30, ret = parseInt(document.getElementById('supSanRetiro').value) || 0; if (!n) { alert('⚠️ Nombre'); return; } DB.suplementosSanidad.push({ id:'supSan_'+Date.now(), nombre:n, dosis:dosis, diasEfecto:de, retiro:ret, stock:0, precioML:0, icono:'fa-syringe', color:'#a78bfa', tipo:'personalizado' }); save(); document.querySelector('.modal-overlay').remove(); renderSanidad(); showToast('<i class="fa-solid fa-check-circle"></i> Agregado'); }
function agregarCompraSanidad(prodId) { var me = document.getElementById('cm-' + prodId), ce = document.getElementById('ccs-' + prodId); if (!me || !ce) return; var ml = parseFloat(me.value), co = parseFloat(ce.value); if (isNaN(ml) || ml <= 0 || isNaN(co) || co <= 0) { alert('⚠️ Datos válidos'); return; } var sp = DB.suplementosSanidad.find(function(s) { return s.id === prodId; }); if (sp) { sp.stock = (sp.stock || 0) + ml; sp.precioML = co / ml; } else { DB.stockSanidad[prodId] = (DB.stockSanidad[prodId] || 0) + ml; DB.preciosSanidad[prodId] = co / ml; } save(); me.value = ''; ce.value = ''; renderSanidad(); showToast('<i class="fa-solid fa-check-circle"></i> Compra registrada'); }
function openAplicarSanidad(animalId) { var a = DB.animales.find(function(x) { return x.id === animalId; }); if (!a) return; var peso = a.historial[a.historial.length-1].peso; var cat = getCatalogoSanidadCompleto(); var opts = ''; for (var i = 0; i < cat.length; i++) opts += '<option value="' + cat[i].id + '">' + cat[i].nombre + '</option>'; var html = '<div class="modal-title"><i class="fa-solid fa-syringe"></i> APLICAR A ' + a.nombre + ' (' + fm(peso) + ' kg)</div><div class="flex-col gap10"><select id="aplProducto" onchange="calcularDosisModal(' + peso + ')">' + opts + '</select><div id="dosisInfo" style="font-size:.7rem;color:var(--muted);"></div><input id="aplML" type="number" placeholder="ml aplicados" step=".1"><button class="btn btn-gold mt8" onclick="aplicarProductoSanidad(' + animalId + ')"><i class="fa-solid fa-check"></i> CONFIRMAR</button><button class="btn btn-gray" onclick="document.querySelector(\'.modal-overlay\').remove()">CANCELAR</button></div>'; showModal(html); setTimeout(function() { calcularDosisModal(peso); }, 100); }
...
function aplicarProductoSanidad(animalId) { var sel = document.getElementById('aplProducto'), mlEl = document.getElementById('aplML'); if (!sel || !mlEl) return; var pid = sel.value, ml = parseFloat(mlEl.value); if (isNaN(ml) || ml <= 0) { alert('⚠️ ml válidos'); return; } var cat = getCatalogoSanidadCompleto(); var p = cat.find(function(x) { return x.id === pid; }); var a = DB.animales.find(function(x) { return x.id === animalId; }); if (!p || !a) return; var prc = p.tipo === 'fijo' ? (DB.preciosSanidad[pid] || 0) : (p.precioML || 0); var ct = prc * ml; DB.aplicaciones.push({ animalId: animalId, productoId: pid, producto: p.nombre, cantidad: ml, unidad: 'ml', costo: ct, fecha: new Date().toLocaleDateString(), tipo: 'sanidad' }); if (p.tipo === 'fijo') { DB.stockSanidad[pid] = Math.max(0, (DB.stockSanidad[pid] || 0) - ml); } else { p.stock = Math.max(0, (p.stock || 0) - ml); } save(); document.querySelector('.modal-overlay').remove(); showToast('<i class="fa-solid fa-check-circle"></i> ' + p.nombre + ': ' + ml + ' ml ($' + fm(ct) + ')'); showProfile(animalId); }

// ==================== INSUMOS ====================
function renderInsumos() { var mez = { pasto:0, salvado:0, sal:0, melaza:0, urea:0, levadura:0, bicarb:0 }; DB.animales.forEach(function(a) { var d = getDietaCompleta(a.historial[a.historial.length-1].peso, a.tipo, a.estadoRepro); for (var k in mez) mez[k] += (d[k] || 0); }); var html = '<div class="card"><div style="font-weight:700;margin-bottom:14px;color:var(--accent);"><i class="fa-solid fa-boxes"></i> ALIMENTOS FIJOS</div>'; for (var i = 0; i < ALIMENTOS.length; i++) { var st = DB.stock[ALIMENTOS[i]] || 0, co = mez[ALIMENTOS[i]] || 0, cr = (ALIMENTOS[i] === 'pasto' || ALIMENTOS[i] === 'salvado') ? co : co/1000; var dias = cr > 0 && st > 0 ? st/cr : 999, dCol = dias < 3 ? '#ef4444' : dias < 7 ? '#f59e0b' : '#22c55e'; html += '<div class="insumo-row"><i class="fa-solid ' + IC_ALIMENTOS[ALIMENTOS[i]] + '"></i><div class="insumo-info"><span class="insumo-nombre">' + NM_ALIMENTOS[ALIMENTOS[i]] + '</span><span class="insumo-detalle">$' + fm(DB.precios[ALIMENTOS[i]] || 0) + '/kg · <b style="color:' + dCol + '">' + (dias === 999 ? '--' : Math.round(dias) + 'd') + '</b></span></div><div class="insumo-inputs"><input id="pr-' + ALIMENTOS[i] + '" type="number" value="' + (DB.precios[ALIMENTOS[i]] || 0) + '"><input id="st-' + ALIMENTOS[i] + '" type="number" value="' + Math.round(st) + '"></div></div>'; } html += '<button class="btn btn-gold mt12" onclick="saveAlimentos()"><i class="fa-solid fa-check"></i> GUARDAR</button></div>'; html += '<div class="card"><div style="font-weight:700;margin-bottom:14px;color:var(--accent);"><i class="fa-solid fa-flask"></i> SUPLEMENTOS (g/kg)</div>'; for (var s = 0; s < DB.suplementosAlimento.length; s++) { var sup = DB.suplementosAlimento[s], stk = sup.stock || 0, dosisEj = (160 * sup.gramosPorKg) / 1000; html += '<div class="sup-card"><div class="sup-card-header"><span class="sup-nombre"><i class="fa-solid ' + (sup.icono || 'fa-flask') + '" style="color:' + (sup.color || '#a78bfa') + ';"></i> ' + sup.nombre + '</span><div class="sup-card-actions"><button onclick="editarSuplementoAlimento(\'' + sup.id + '\')"><i class="fa-solid fa-pen-to-square"></i></button><button onclick="eliminarSuplementoAlimento(\'' + sup.id + '\')"><i class="fa-solid fa-trash"></i></button></div></div><div class="sup-card-body"><span>📐 ' + sup.gramosPorKg + ' g/kg</span><span>💰 $' + fm(sup.precioPorKg || 0) + '/kg</span><span>📦 ' + fm(stk) + ' kg</span><span>📋 160kg→' + dosisEj.toFixed(2) + ' kg/d</span></div><div style="display:flex;gap:4px;margin-top:6px;"><input id="ccs-' + sup.id + '" type="number" placeholder="kg" style="flex:1;padding:6px;font-size:.65rem;"><input id="ccos-' + sup.id + '" type="number" placeholder="Costo $" style="flex:1;padding:6px;font-size:.65rem;"><button class="btn btn-green btn-sm" onclick="comprarSuplementoAlimento(\'' + sup.id + '\')" style="padding:4px 8px;"><i class="fa-solid fa-cart-shopping"></i></button></div></div>'; } html += '<button class="btn btn-purple mt12" onclick="openAgregarSuplementoAlimento()"><i class="fa-solid fa-plus"></i> AGREGAR SUPLEMENTO</button></div>'; document.getElementById('v-insumos').innerHTML = html; }
function saveAlimentos() { for (var i = 0; i < ALIMENTOS.length; i++) { var pel = document.getElementById('pr-' + ALIMENTOS[i]), sel = document.getElementById('st-' + ALIMENTOS[i]); if (pel) DB.precios[ALIMENTOS[i]] = parseFloat(pel.value) || 0; if (sel) DB.stock[ALIMENTOS[i]] = parseFloat(sel.value) || 0; } save(); showToast('<i class="fa-solid fa-check-circle"></i> Guardado'); }
function openAgregarSuplementoAlimento() { var html = '<div class="modal-title"><i class="fa-solid fa-flask"></i> NUEVO SUPLEMENTO (g/kg)</div><div class="flex-col gap10"><input id="supAlimNombre" type="text" placeholder="Nombre"><input id="supAlimGramos" type="number" placeholder="g/kg de peso vivo" step="1" value="50"><input id="supAlimPrecio" type="number" placeholder="Precio por kg ($)" step="1"><button class="btn btn-purple mt8" onclick="agregarSuplementoAlimento()"><i class="fa-solid fa-check"></i> AGREGAR</button><button class="btn btn-gray" onclick="document.querySelector(\'.modal-overlay\').remove()">CANCELAR</button></div>'; showModal(html); }
function agregarSuplementoAlimento() { var n = document.getElementById('supAlimNombre').value.trim(), g = parseInt(document.getElementById('supAlimGramos').value) || 50, pr = parseFloat(document.getElementById('supAlimPrecio').value) || 0; if (!n) { alert('⚠️ Nombre'); return; } DB.suplementosAlimento.push({ id:'supAlim_'+Date.now(), nombre:n, gramosPorKg:g, precioPorKg:pr, stock:0, icono:'fa-flask', color:'#a78bfa' }); save(); document.querySelector('.modal-overlay').remove(); renderInsumos(); showToast('<i class="fa-solid fa-check-circle"></i> Agregado'); }
function editarSuplementoAlimento(id) { var sup = DB.suplementosAlimento.find(function(s) { return s.id === id; }); if (!sup) return; var html = '<div class="modal-title"><i class="fa-solid fa-pen-to-square"></i> EDITAR ' + sup.nombre + '</div><div class="flex-col gap10"><input id="editSupNombre" type="text" value="' + sup.nombre + '"><input id="editSupGramos" type="number" value="' + sup.gramosPorKg + '"><input id="editSupPrecio" type="number" value="' + (sup.precioPorKg || 0) + '"><button class="btn btn-gold mt8" onclick="guardarEdicionSuplemento(\'' + id + '\')"><i class="fa-solid fa-check"></i> GUARDAR</button><button class="btn btn-gray" onclick="document.querySelector(\'.modal-overlay\').remove()">CANCELAR</button></div>'; showModal(html); }
function guardarEdicionSuplemento(id) { var sup = DB.suplementosAlimento.find(function(s) { return s.id === id; }); if (!sup) return; sup.nombre = document.getElementById('editSupNombre').value.trim(); sup.gramosPorKg = parseInt(document.getElementById('editSupGramos').value) || 50; sup.precioPorKg = parseFloat(document.getElementById('editSupPrecio').value) || 0; save(); document.querySelector('.modal-overlay').remove(); renderInsumos(); showToast('<i class="fa-solid fa-check-circle"></i> Actualizado'); }
function eliminarSuplementoAlimento(id) { if (confirm('⚠️ ¿Eliminar?')) { DB.suplementosAlimento = DB.suplementosAlimento.filter(function(s) { return s.id !== id; }); save(); renderInsumos(); showToast('<i class="fa-solid fa-check-circle"></i> Eliminado'); } }
function comprarSuplementoAlimento(id) { var ce = document.getElementById('ccs-' + id), coe = document.getElementById('ccos-' + id); if (!ce || !coe) return; var kg = parseFloat(ce.value), costo = parseFloat(coe.value); if (isNaN(kg) || kg <= 0 || isNaN(costo) || costo <= 0) { alert('⚠️ Datos válidos'); return; } var sup = DB.suplementosAlimento.find(function(s) { return s.id === id; }); if (!sup) return; sup.stock = (sup.stock || 0) + kg; if (!sup.precioPorKg || sup.precioPorKg === 0) sup.precioPorKg = costo / kg; save(); ce.value = ''; coe.value = ''; renderInsumos(); showToast('<i class="fa-solid fa-check-circle"></i> Stock +' + fm(kg) + ' kg'); }

// ==================== AJUSTES ====================
function renderAjustes() { var lotesHTML = ''; for (var i = 0; i < DB.lotes.length; i++) { var l = DB.lotes[i]; var count = DB.animales.filter(function(a) { return a.lote === l.id; }).length; lotesHTML += '<div style="display:flex;align-items:center;gap:8px;padding:8px 0;border-bottom:1px solid rgba(255,255,255,.03);"><i class="fa-solid ' + (l.tipo === 'engorde' ? 'fa-cow' : 'fa-glass-water-droplet') + '" style="color:var(--accent);"></i><span style="flex:1;font-size:.75rem;">' + l.nombre + ' (' + count + ')</span><button class="btn btn-danger btn-sm" onclick="eliminarLote(\'' + l.id + '\')" style="padding:4px 8px;"><i class="fa-solid fa-trash"></i></button></div>'; } var html = '<div class="card config-section"><h3><i class="fa-solid fa-layer-group"></i> LOTES</h3>' + lotesHTML + '<button class="btn btn-purple btn-sm mt8" onclick="openCrearLote()" style="width:100%;"><i class="fa-solid fa-plus"></i> CREAR LOTE</button></div>' + '<div class="card config-section"><h3><i class="fa-solid fa-database"></i> RESPALDO</h3><button class="btn btn-gold" onclick="exportarDatos()"><i class="fa-solid fa-download"></i> EXPORTAR</button><button class="btn btn-gray" onclick="importarDatos()"><i class="fa-solid fa-upload"></i> IMPORTAR</button></div>' + '<div class="card config-section"><h3><i class="fa-solid fa-mobile-screen"></i> INSTALAR</h3><button class="btn btn-gold" onclick="instalarApp()"><i class="fa-solid fa-download"></i> INSTALAR PWA</button></div>' + '<div class="card config-section"><h3><i class="fa-solid fa-info-circle"></i> INFORMACIÓN</h3><p style="font-size:.7rem;color:var(--muted);">GANADERO ÉLITE v5.0</p><p style="font-size:.6rem;color:var(--muted);">🥩 Engorde + 🥛 Leche · 🧠 IA Predictiva</p><p style="font-size:.6rem;color:var(--muted);">📡 100% Offline · Auto-guardado 15s</p><p style="font-size:.6rem;color:var(--muted);">Animales: ' + DB.animales.length + ' · Lotes: ' + (DB.lotes?DB.lotes.length:0) + '</p></div>'; document.getElementById('v-ajustes').innerHTML = html; }
function openCrearLote() { var html = '<div class="modal-title"><i class="fa-solid fa-layer-group"></i> NUEVO LOTE</div><div class="flex-col gap10"><input id="loteNombre" type="text" placeholder="Nombre del lote"><select id="loteTipo"><option value="engorde">🥩 Engorde (Carne)</option><option value="leche">🥛 Leche</option></select><button class="btn btn-gold mt8" onclick="crearLote()"><i class="fa-solid fa-check"></i> CREAR</button><button class="btn btn-gray" onclick="document.querySelector(\'.modal-overlay\').remove()">CANCELAR</button></div>'; showModal(html); }
function crearLote() { var n = document.getElementById('loteNombre').value.trim(), t = document.getElementById('loteTipo').value; if (!n) { alert('⚠️ Nombre del lote'); return; } DB.lotes.push({ id: 'lote_' + Date.now(), nombre: n, tipo: t }); save(); document.querySelector('.modal-overlay').remove(); renderAjustes(); showToast('<i class="fa-solid fa-check-circle"></i> Lote creado'); }
function eliminarLote(id) { if (confirm('⚠️ ¿Eliminar este lote? Los animales quedarán sin lote.')) { DB.lotes = DB.lotes.filter(function(l) { return l.id !== id; }); DB.animales.forEach(function(a) { if (a.lote === id) a.lote = null; }); save(); renderAjustes(); showToast('<i class="fa-solid fa-check-circle"></i> Lote eliminado'); } }
function exportarDatos() { var b = new Blob([JSON.stringify(DB,null,2)],{type:'application/json'}); var a = document.createElement('a'); a.href = URL.createObjectURL(b); a.download = 'ganadero-elite-respaldo.json'; a.click(); showToast('<i class="fa-solid fa-check-circle"></i> Exportado'); }
function importarDatos() { var i = document.createElement('input'); i.type = 'file'; i.accept = '.json'; i.onchange = function(e) { var r = new FileReader(); r.onload = function(e) { try { DB = JSON.parse(e.target.result); save(); renderDashboard(); showToast('<i class="fa-solid fa-check-circle"></i> Importado'); } catch(err) { alert('❌ Error'); } }; r.readAsText(e.target.files[0]); }; i.click(); }
...
console.log('✅ GANADERO ÉLITE v5.0 UNIFICADO - Listo');
