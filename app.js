console.log('GANADERO ÉLITE v3.2 · Carne + Leche');
...
function cargarDatos() { try { var s = localStorage.getItem('ganadero_elite_v7'); if (s) { DB = JSON.parse(s); return; } } catch(e) {} }
function save() { try { localStorage.setItem('ganadero_elite_v7', JSON.stringify(DB)); } catch(e) {} }
...
// ==================== FÓRMULAS ====================
function getEtapa(pv, tipo) {
    if (tipo === 'leche') {
        if (pv < 350) return 'Novilla';
        return 'Parida'; // Por defecto, el estado reproductivo lo maneja aparte
    }
...
function getDietaCompleta(pv, tipo, estadoRepro) {
    ...
    // Para leche, el estado reproductivo determina la etapa real
    ...
    return {
        ...
        melaza: consumoTotal * (m.melaza / 100),
...
function addAnimal() {
    ...
    save(); closeAddModal(); renderLotes(); showToast('✅ ' + n + ' registrado');
}
// ==================== RENDER LOTES (DASHBOARD) ====================
function renderLotes() {
    var html = '';
    // Si no hay lotes, mostrar todos los animales juntos
    if (DB.lotes.length === 0) {
        html += '<div class="card card-sm"><div style="font-weight:700;font-size:.7rem;color:var(--muted);">📋 TODOS LOS ANIMALES</div></div>';
        html += renderDashboardGeneral();
    } else {
        // Mostrar tarjetas de lotes
        for (var i = 0; i < DB.lotes.length; i++) {
            var lote = DB.lotes[i];
            var animalesLote = DB.animales.filter(function(a) { return a.lote === lote.id; });
            var totalKg = 0; for (var j = 0; j < animalesLote.length; j++) totalKg += animalesLote[j].historial[animalesLote[j].historial.length-1].peso;
            var produccion = ''; if (lote.tipo === 'leche') { var litros = 0; animalesLote.forEach(function(a) { if (a.produccionLeche && a.produccionLeche.length > 0) litros += a.produccionLeche[a.produccionLeche.length-1].litros; }); produccion = litros > 0 ? ' · ' + litros + ' L/día' : ''; }
            html += '<div class="lote-card" onclick="verLote(\'' + lote.id + '\')"><div class="lote-nombre">' + (lote.tipo === 'engorde' ? '🥩 ' : '🥛 ') + lote.nombre + '</div><div class="lote-info">' + animalesLote.length + ' animales · ' + fm(totalKg) + ' kg' + produccion + '</div></div>';
        }
        html += '<button class="btn btn-purple btn-sm mt8" onclick="openCrearLote()" style="width:100%;"><i class="fa-solid fa-plus"></i> CREAR LOTE</button>';
        // También mostrar resumen general
        html += '<div class="section-title"><i class="fa-solid fa-chart-pie"></i> RESUMEN GENERAL</div>';
        html += renderDashboardGeneral();
    }
...
function renderDashboardGeneral() {
    ...
    for (var i = 0; i < DB.animales.length; i++) {
        ...
        var gmd = getGMD(a.historial); if (!mejorA || gmd > mejorA.gmd) mejorA = { nombre: a.nombre, gmd: gmd }; if (!peorA || gmd < peorA.gmd) peorA = { nombre: a.nombre, gmd: gmd };
        ...
        var ret = false; var cat = getCatalogoSanidadCompleto(); for (var j = DB.aplicaciones.length-1; j >= 0; j--) { if (DB.aplicaciones[j].animalId === a.id) { var pr = cat.find(function(p) { return p.id === DB.aplicaciones[j].productoId; }); if (pr && pr.retiro > 0 && getDiasDesde(DB.aplicaciones[j].fecha) < pr.retiro) { ret = true; break; } } }
        var semaforo = getSemaforo(a); var semaforoHTML = semaforo ? '<span class="semaforo semaforo-' + semaforo.color + '"></span>' : '';
        cards += '<div class="animal-card ' + etapa.cardClass + '" onclick="showProfile(' + a.id + ')"><div class="mini-led ' + lm[r.nivel] + '"></div>' + (etapa.ureaBloqueada ? '<div class="lock-icon"><i class="fa-solid fa-lock"></i></div>' : '') + '<span style="font-size:1.5rem;">' + etapa.icono + '</span><div class="name">' + a.nombre + ' ' + semaforoHTML + '</div><span class="etapa-tag ' + etapa.clase + '">' + etapa.rango + '</span><div class="weight">' + fm(cp) + ' kg ' + tendHTML + '</div>' + (ret ? '<div class="retiro-badge">🚫 EN VEDA</div>' : '') + '<div class="cm" style="color:' + (r.cm >= 0 ? '#22c55e' : '#ef4444') + '">' + sg + r.cm.toFixed(1) + '%</div></div>';
    }
    ...
    var iaHTML = ''; if (pesoProy30 > 0) { var gan30 = valorProy30 - (totalKg * price); iaHTML = '<div class="card card-sm"><div style="font-weight:700;font-size:.68rem;margin-bottom:6px;color:var(--muted);"><i class="fa-solid fa-brain"></i> PREDICCIÓN IA</div><div class="row"><span class="row-label">Peso en 30d</span><span class="row-val">' + fm(pesoProy30) + ' kg</span></div><div class="row"><span class="row-label">Valor en 30d</span><span class="row-val" style="color:var(--accent);">$ ' + fm(valorProy30) + '</span></div><div class="row"><span class="row-label">Ganancia est.</span><span class="row-val" style="color:' + (gan30 >= 0 ? '#22c55e' : '#ef4444') + '">' + (gan30 >= 0 ? '+' : '') + '$ ' + fm(gan30) + '</span></div>'; if (mejorA) iaHTML += '<div class="ranking-item"><span class="ranking-emoji">🏆</span> Mejor: <b>' + mejorA.nombre + '</b></div>'; if (peorA && ta > 1) iaHTML += '<div class="ranking-item"><span class="ranking-emoji">⚠️</span> Atención: <b>' + peorA.nombre + '</b></div>'; iaHTML += '</div>'; }
    ...
    return '<div class="card card-sm"><div style="display:flex;align-items:center;gap:6px;"><span style="font-size:1.1rem;font-weight:800;color:var(--accent);">$</span><input id="inpPKG" type="number" value="' + price + '" style="font-size:1.1rem;font-weight:700;text-align:center;"><span style="font-size:.7rem;color:var(--muted);">COP</span></div><button class="btn btn-green mt8" onclick="savePKG()"><i class="fa-solid fa-check"></i> ACTUALIZAR</button></div>' +
        '<div class="card card-sm"><div style="font-weight:600;"><i class="fa-solid fa-chart-pie"></i> CAPITAL</div><div class="capital-value">$ ' + fm(totalKg * price) + '</div><div class="stats-grid"><div class="stat-item"><div class="row-label"><i class="fa-solid fa-users"></i> Cabezas</div><div class="row-val">' + ta + '</div></div><div class="stat-item"><div class="row-label"><i class="fa-solid fa-weight-scale"></i> Peso Total</div><div class="row-val">' + fm(totalKg) + ' kg</div></div></div></div>' +
        iaHTML +
        '<div class="card card-sm"><div style="font-weight:700;font-size:.68rem;margin-bottom:6px;color:var(--muted);"><i class="fa-solid fa-chart-simple"></i> ESTADO</div><div class="estado-simple"><div class="estado-pildora e"><div class="num">' + est.verde + '</div><div class="lbl">Excelente</div></div><div class="estado-pildora b"><div class="num">' + est.azul + '</div><div class="lbl">Bueno</div></div><div class="estado-pildora r"><div class="num">' + est.naranja + '</div><div class="lbl">Regular</div></div><div class="estado-pildora m"><div class="num">' + est.rojo + '</div><div class="lbl">Bajo</div></div></div><div class="progress"><div class="progress-fill" style="width:' + pctB + '%;background:var(--info);"></div></div></div>' +
        ...
        '<div class="section-title"><i class="fa-solid fa-layer-group"></i> ANIMALES</div><div class="grid">' + cards + '</div>';
}
...
function verLote(loteId) {
    var lote = DB.lotes.find(function(l) { return l.id === loteId; }); if (!lote) return;
    var animalesLote = DB.animales.filter(function(a) { return a.lote === loteId; });
    var totalKg = 0; for (var i = 0; i < animalesLote.length; i++) totalKg += animalesLote[i].historial[animalesLote[i].historial.length-1].peso;
    var cards = '';
    for (var i = 0; i < animalesLote.length; i++) {
        var a = animalesLote[i], cp = a.historial[a.historial.length-1].peso;
        var etapa = getEtapaCompleta(cp, a.tipo, a.estadoRepro);
        var r = getRendimiento(a.historial);
        var lm = { verde:'ml-g', azul:'ml-b', naranja:'ml-o', rojo:'ml-r', gris:'ml-x' }, sg = r.cm >= 0 ? '+' : '';
        var semaforo = getSemaforo(a); var semaforoHTML = semaforo ? '<span class="semaforo semaforo-' + semaforo.color + '"></span>' : '';
        cards += '<div class="animal-card ' + etapa.cardClass + '" onclick="showProfile(' + a.id + ')"><div class="mini-led ' + lm[r.nivel] + '"></div><span style="font-size:1.5rem;">' + etapa.icono + '</span><div class="name">' + a.nombre + ' ' + semaforoHTML + '</div><span class="etapa-tag ' + etapa.clase + '">' + etapa.rango + '</span><div class="weight">' + fm(cp) + ' kg</div><div class="cm" style="color:' + (r.cm >= 0 ? '#22c55e' : '#ef4444') + '">' + sg + r.cm.toFixed(1) + '%</div></div>';
    }
    var html = '<div class="card"><div style="display:flex;justify-content:space-between;align-items:center;"><div><div style="font-weight:700;font-size:.9rem;color:var(--accent);">' + (lote.tipo === 'engorde' ? '🥩 ' : '🥛 ') + lote.nombre + '</div><div style="font-size:.65rem;color:var(--muted);">' + animalesLote.length + ' animales · ' + fm(totalKg) + ' kg</div></div><button class="btn btn-gray btn-sm" onclick="renderLotes()">← Volver</button></div></div><div class="section-title">ANIMALES</div><div class="grid">' + cards + '</div>';
    document.getElementById('v-lote').innerHTML = html;
}

// ==================== CREAR LOTE ====================
function openCrearLote() {
    var html = '<div class="modal-title">📦 NUEVO LOTE</div><div class="flex-col gap10"><input id="loteNombre" type="text" placeholder="Nombre del lote"><select id="loteTipo"><option value="engorde">🥩 Engorde (Carne)</option><option value="leche">🥛 Leche</option></select><button class="btn btn-gold mt8" onclick="crearLote()"><i class="fa-solid fa-check"></i> CREAR</button><button class="btn btn-gray" onclick="document.querySelector(\'.modal-overlay\').remove()">CANCELAR</button></div>';
    showModal(html);
}
function crearLote() {
    var n = document.getElementById('loteNombre').value.trim(), t = document.getElementById('loteTipo').value;
    if (!n) { alert('⚠️ Nombre del lote'); return; }
    DB.lotes.push({ id: 'lote_' + Date.now(), nombre: n, tipo: t });
    save(); document.querySelector('.modal-overlay').remove(); renderLotes(); showToast('✅ Lote creado');
}
// ==================== PERFIL DEL ANIMAL ====================
function showProfile(id) {
    ...
    var edadHTML = ''; if (a.origen === 'nacimiento' && a.fechaNacimiento) { var diasEdad = getDiasDesde(a.fechaNacimiento); var meses = Math.floor(diasEdad / 30); edadHTML = '<div class="row"><span class="row-label"><i class="fa-solid fa-cake-candles"></i> Edad</span><span class="row-val">' + meses + ' meses (' + diasEdad + ' días)</span></div>'; }
    ...
    var criasHTML = '';
    if (a.tipo === 'leche') {
        var crias = DB.animales.filter(function(x) { return x.madre === a.nombre || x.madre === a.id; });
        if (crias.length > 0) { criasHTML = '<div class="section-title"><i class="fa-solid fa-baby"></i> CRÍAS (' + crias.length + ')</div>'; for (var c = 0; c < crias.length; c++) criasHTML += '<div class="row" style="cursor:pointer;" onclick="showProfile(' + crias[c].id + ')"><span class="row-label"><i class="fa-solid ' + (crias[c].tipo === 'engorde' ? 'fa-cow' : 'fa-cow') + '"></i> ' + crias[c].nombre + '</span><span class="row-val">' + fm(crias[c].historial[crias[c].historial.length-1].peso) + ' kg</span></div>'; }
    }
    ...
    // Botones dinámicos según etapa
    var botonesHTML = '';
    // Cambiar a Engorde (si es vaca)
    if (a.tipo === 'leche' && (a.estadoRepro === 'seca' || a.estadoRepro === 'parida')) {
        botonesHTML += '<button class="btn btn-gray btn-sm" onclick="cambiarAEngorde(' + id + ')" style="flex:1;"><i class="fa-solid fa-rotate"></i> Cambiar a Engorde</button>';
    }
    // Quedó preñada (si está en ventana)
    if (a.tipo === 'leche' && a.estadoRepro === 'parida' && semaforo && semaforo.dias >= 60 && !a.fechaPrenez) {
        botonesHTML += '<button class="btn btn-purple btn-sm" onclick="quedoPrenada(' + id + ')" style="flex:1;"><i class="fa-solid fa-baby"></i> Quedó Preñada</button>';
    }
    // Iniciar secado (si está preñada y cerca la fecha)
    if (a.tipo === 'leche' && a.fechaPrenez && a.fechaSecado) {
        var diasParaSecado = getDiasDesde(a.fechaSecado);
        if (diasParaSecado <= 7 && a.estadoRepro === 'parida') {
            botonesHTML += '<button class="btn btn-warning btn-sm" onclick="iniciarSecado(' + id + ')" style="flex:1;"><i class="fa-solid fa-pause"></i> Iniciar Secado</button>';
        }
    }
    // Ya parió (si está seca)
    if (a.tipo === 'leche' && a.estadoRepro === 'seca') {
        botonesHTML += '<button class="btn btn-green btn-sm" onclick="yaPario(' + id + ')" style="flex:1;"><i class="fa-solid fa-calendar-check"></i> Ya Parió</button>';
    }
    ...
    var html = '<div class="card"><div class="profile-cover"><div class="profile-avatar" id="fotoPerfil" onclick="abrirFoto(' + id + ')">' + (a.foto ? '<img src="' + a.foto + '" alt="Foto"><div class="foto-overlay">📸 Cambiar</div>' : etapa.icono + '<div class="foto-overlay">📸 Agregar</div>') + '</div><div class="profile-name">' + a.nombre + '</div><div class="profile-sub">' + etapa.rango + ' · ' + (a.tipo === 'engorde' ? '🥩 Engorde' : '🥛 Leche');
...
function abrirFoto(id) {
    ...
    input.setAttribute('data-animal', id);
    input.click();
}
function guardarFoto() {
    var input = document.getElementById('fotoInput');
    var id = parseInt(input.getAttribute('data-animal'));
    var file = input.files[0]; if (!file) return;
    var reader = new FileReader();
    reader.onload = function(e) {
        var a = DB.animales.find(function(x) { return x.id === id; }); if (!a) return;
        a.foto = e.target.result; save(); showProfile(id); showToast('📸 Foto guardada');
    };
    reader.readAsDataURL(file);
}

// ==================== REGISTRAR LECHE ====================
function registrarLeche(id) {
    var a = DB.animales.find(function(x) { return x.id === id; }); if (!a) return;
    var litrosHoy = a.produccionLeche && a.produccionLeche.length > 0 ? a.produccionLeche[a.produccionLeche.length-1].litros : 0;
    var html = '<div class="modal-title">🥛 REGISTRAR LECHE - ' + a.nombre + '</div><div class="flex-col gap10"><input id="lecheLitros" type="number" placeholder="Litros producidos hoy" step="0.5" value="' + litrosHoy + '"><input id="lechePrecio" type="number" placeholder="Precio por litro ($)" value="' + (DB.litroLeche || 1500) + '"><button class="btn btn-gold mt8" onclick="guardarLeche(' + id + ')"><i class="fa-solid fa-check"></i> GUARDAR</button><button class="btn btn-gray" onclick="document.querySelector(\'.modal-overlay\').remove()">CANCELAR</button></div>';
    showModal(html);
}
function guardarLeche(id) {
    var litros = parseFloat(document.getElementById('lecheLitros').value);
    var precio = parseFloat(document.getElementById('lechePrecio').value);
    if (isNaN(litros) || litros < 0) { alert('⚠️ Litros válidos'); return; }
    if (precio && !isNaN(precio)) DB.litroLeche = precio;
    var a = DB.animales.find(function(x) { return x.id === id; }); if (!a) return;
    if (!a.produccionLeche) a.produccionLeche = [];
    a.produccionLeche.push({ fecha: new Date().toLocaleDateString(), litros: litros });
    save(); document.querySelector('.modal-overlay').remove(); showProfile(id); showToast('✅ ' + litros + ' L registrados');
}

// ==================== FUNCIONES REPRODUCTIVAS ====================
function quedoPrenada(id) {
    if (!confirm('🤰 ¿Confirmar que esta vaca quedó preñada?\n\nSe calculará la fecha de parto (285 días) y la fecha de secado (60 días antes).')) return;
    var a = DB.animales.find(function(x) { return x.id === id; }); if (!a) return;
    var hoy = new Date();
    a.fechaPrenez = hoy.toLocaleDateString();
    var parto = new Date(hoy.getTime() + 285 * 86400000);
    a.fechaPartoEstimada = parto.toLocaleDateString();
    var secado = new Date(parto.getTime() - 60 * 86400000);
    a.fechaSecado = secado.toLocaleDateString();
    save(); showProfile(id); showToast('✅ Preñez registrada · Parto: ' + a.fechaPartoEstimada);
}
function iniciarSecado(id) {
    if (!confirm('🔴 ¿Iniciar secado de esta vaca?\n\nCambiará a dieta de Seca (1% Melaza, 0.20g Sal).')) return;
    var a = DB.animales.find(function(x) { return x.id === id; }); if (!a) return;
    a.estadoRepro = 'seca';
    a.fechaSecadoInicio = new Date().toLocaleDateString();
    save(); showProfile(id); showToast('✅ Secado iniciado · Dieta ajustada');
}
function yaPario(id) {
    if (!confirm('✅ ¿Confirmar que esta vaca ya parió?\n\nSe reiniciará el ciclo reproductivo.')) return;
    var a = DB.animales.find(function(x) { return x.id === id; }); if (!a) return;
    a.estadoRepro = 'parida';
    a.fechaParto = new Date().toLocaleDateString();
    a.fechaPrenez = null; a.fechaPartoEstimada = null; a.fechaSecado = null; a.fechaSecadoInicio = null;
    if (confirm('🐮 ¿Desea registrar la cría recién nacida?')) {
        var pesoCria = parseFloat(prompt('⚖️ Peso de la cría al nacer (kg):'));
        if (!isNaN(pesoCria) && pesoCria > 10) {
            var nombreCria = prompt('📝 Nombre de la cría:') || ('Cría de ' + a.nombre);
            var idCria = Date.now() + 1;
            DB.animales.push({ id: idCria, nombre: nombreCria, tipo: 'leche', origen: 'nacimiento', madre: a.nombre, fechaNacimiento: new Date().toLocaleDateString(), historial: [{ fecha: new Date().toLocaleDateString(), peso: pesoCria }], estadoRepro: 'novilla', lote: a.lote, produccionLeche: [] });
        }
    }
    save(); showProfile(id); showToast('✅ Parto registrado · Ciclo reiniciado');
}
function cambiarAEngorde(id) {
    if (!confirm('🔄 ¿Cambiar esta vaca a ENGORDE para venta?\n\nSe aplicará dieta de ceba (5% Melaza, 0.11g Urea).')) return;
    var a = DB.animales.find(function(x) { return x.id === id; }); if (!a) return;
    a.tipo = 'engorde'; a.estadoRepro = 'venta';
    save(); showProfile(id); showToast('✅ Cambiada a Engorde · Dieta ajustada');
}
...
function importarDatos() { var i = document.createElement('input'); i.type = 'file'; i.accept = '.json'; i.onchange = function(e) { var r = new FileReader(); r.onload = function(e) { try { DB = JSON.parse(e.target.result); save(); renderLotes(); showToast('✅ Importado'); } catch(err) { alert('❌ Error'); } }; r.readAsText(e.target.files[0]); }; i.click(); }

// ==================== AUTO-GUARDADO ====================
cargarDatos(); renderLotes();
setTimeout(function() { var s = document.getElementById('splash'); if (s && !s.classList.contains('hide')) s.classList.add('hide'); }, 1000);
window.addEventListener('beforeunload', function() { save(); });
document.addEventListener('visibilitychange', function() { if (document.hidden) save(); });
setInterval(function() { save(); }, 30000);
console.log('✅ GANADERO ÉLITE v3.2 listo');
