...
import { db } from './db'
import {
  Animal, Pesaje, fm, getGMD, getDietaCompleta, getCostoDiario, getRendimiento,
...
const Perfil: React.FC<Props> = ({ animalId, precioKG, litroLeche, preciosAlimento, volver, recargar }) => {
    ...
    if (animal.origen === 'nacimiento' && animal.fechaNacimiento) {
      ...
      parts.push(
        <div className="row" key="edad">
          <span className="row-label"><Icono nombre="cake" tamaño={14} /> Edad</span>
    ...
    if (animal.origen === 'comprado' && animal.precioCompra) {
      ...
      parts.push(
        <div className="row" key="roi">
          <span className="row-label"><Icono nombre="currency-dollar" tamaño={14} /> ROI</span>
  ...
  const registrarPesaje = async () => {
    ...
    if (detectarAnomalia(animal.historial, peso)) {
      if (!confirm('⚠️ El peso ' + peso + 'kg está muy lejos de la tendencia.\n¿Registrar de todos modos?')) return
    }
  ...
  return (
    <div>
      ...
      <div className="card">
        <div className="profile-cover">
          ...
          <div className="profile-sub">
            ...
            {semaforo && <span className={`semaforo semaforo-${semaforo.color || 'verde'} ml-1`} />}
            {semaforo && <span className="text-xs ml-1">{semaforo.texto}</span>}
          </div>
      ...
      {animal.tipo === 'leche' && animal.estadoRepro === 'parida' && (
        <div className="card-sm mb-3" style={{ background: 'rgba(245,200,66,0.05)', borderColor: 'rgba(245,200,66,0.2)' }}>
          <div className="font-bold text-xs text-accent mb-2">🥛 PRODUCCIÓN LECHE</div>
          <div className="row">
      ...
      {crias.length > 0 && (
        <div className="card-sm mb-3">
          <div className="section-title"><Icono nombre="face-smile" tamaño={12} /> CRÍAS ({crias.length})</div>
          {crias.map(c => (
      ...
      {hayIA ? (
        <div className="ia-card">
          <div className="ia-title"><Icono nombre="sparkles" tamaño={14} variante="solid" /> PREDICCIÓN IA · {confianza}</div>
        ...
        <div className="card-sm mb-3">
          <div className="font-bold text-xs text-text-muted mb-2">📈 PROYECCIÓN SIMPLE</div>
          ...
          <div className="text-[0.55rem] text-text-muted mt-1">⚠️ 3+ pesajes para IA</div>
        </div>
      )}
      ...
      <div className="flex gap-2 mb-3">
        ...
        <button className="btn btn-purple btn-sm flex-1" onClick={aplicarSanidad}>
          <Icono nombre="beaker" tamaño={14} /> Sanidad
        </button>
        <button className="btn btn-green btn-sm flex-[2]" onClick={registrarPesaje}>
          <Icono nombre="scale" tamaño={14} /> PESAJE
        </button>
      </div>
      ...
      {/* Eliminar */}
      <button className="btn btn-danger btn-sm w-full mb-3" onClick={eliminarAnimal}>
        <Icono nombre="trash" tamaño={14} /> ELIMINAR ANIMAL
      </button>

      {/* Volver */}
      <button className="btn btn-gray w-full" onClick={volver}>
        <Icono nombre="arrow-up-tray" tamaño={14} /> VOLVER
      </button>
    </div>
  )
}
